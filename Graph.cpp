...
const Airport Graph::InvalidAirport = "_CS225INVALIDAirport";
     ...
     random(Random(seed)) 
{
    ...
    {
        insertAirport(to_string(i));
        vertices.push_back(to_string(i));
    }

    // make sure all vertices are connected
    random.shuffle(vertices);
    Airport cur = vertices[0];
    for (size_t i = 0; i < vertices.size() - 1; ++i)
    {
        Airport next = vertices[i + 1];
        insertEdge(cur, next);
        if (weighted) 
        {
...
bool Graph::AirportExists(Airport v) const
{
    return assertAirportExists(v, "");
}
...
void Graph::savePNG(string title) const
{
    ...
    vector<Airport> allv = getVertices();
    //lambda expression
    ...
    for (auto it = adjacency_list.begin(); it != adjacency_list.end(); ++it) 
    {
        for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) 
        {
            string Airport1Text = it->first;
            string Airport2Text = it2->first;
